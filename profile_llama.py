...
from models.llama_palu import LlamaForCausalLM_PALU, LlamaModel_PALU
...
def profile_tpot(model, cache_size_k, cache_size_v, cache_type=torch.float16, batch_size=1, prompt_len=1024, repeats=100,
    ...
    input_token = torch.randint(100, (batch_size, 1), dtype=torch.long, device=device) # only input 1 token at a time
    ...
    new_input_token = torch.randint(100, (batch_size, 1), dtype=torch.long, device=device) # only input 1 token at a time
    ...
    with torch.no_grad():
        ...
        for _ in range(repeats):
            generate(new_input_token, past_key_values=past_key_values)
        end.record()
...
def main(args):    
    ...
    if args.palu:
        attention, config = build_llama_palu(args)
        attention.eval()
        ...
        attention, config = build_llama(args)
        attention.eval()
