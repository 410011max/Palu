...
class LlamaAttention_PALU_no_rope(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # self.rank_k = [128, 128, 128, 128, 128, 128, 128, 128]
        # self.rank_v = [128, 128, 128, 128, 128, 128, 128, 128]
        # self.group_size = 4
        # self.num_groups = 8
        self.rank_k = config.rank_k
        self.rank_v = config.rank_v
        ...
        self.fused_group_dim = self.rank_v // self.num_groups
        self.fused_hidden_dim = self.fused_group_dim * self.num_heads
        print(f'fused_group_dim: {self.fused_group_dim}')
        print(f'fused_hidden_dim: {self.fused_hidden_dim}')
        self.q_proj = nn.Linear(self.hidden_size, self.fused_hidden_dim, bias=config.attention_bias)
        self.k_proj = nn.Linear(self.hidden_size, self.rank_k, bias=config.attention_bias)
        self.v_proj = nn.Linear(self.hidden_size, self.rank_v, bias=config.attention_bias)
        self.o_proj = nn.Linear(self.fused_hidden_dim, self.hidden_size, bias=config.attention_bias)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # key_states = self.k_proj(hidden_states)
        # value_states = self.v_proj(hidden_states)
        key_h_states = self.k_proj(hidden_states)
        value_h_states = self.v_proj(hidden_states)

        # query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        # key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        # value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        query_states = query_states.view(bsz, q_len, self.num_heads, self.fused_group_dim).transpose(1, 2)
        key_h_states = key_h_states.view(bsz, q_len, self.num_groups, self.fused_group_dim).transpose(1, 2)
